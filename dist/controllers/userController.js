    ...
    try {
        ...
        const titleResponse = await clientOpenAI.chat.completions.create({
            ...
            messages: [
                {
                    role: "system",
                    content: "Generate a short clearer name for my project from the user's website request." +
                        "Return only the project name, 3-6 words. Do not give quotes.",
                },
                {
                    role: "user",
        ...
        const projectName = titleResponse.choices[0].message.content?.trim() || "Untitled Project";
        if (projectName === null) {
            console.log("Content is null");
        }
        else {
            console.log("Content is a string:", projectName);
        }
    ...
    try {
        ...
        res.json({
            message: project.isPublished
                ? "Project has been not published!"
